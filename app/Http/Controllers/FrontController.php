...
class FrontController extends Controller
{
    public function __construct()
    {
        ...
        if (Cache::has('fine_jewelry')) {
            ...
            $fine_jewelry = Collection::where('parent', 3)->where('status', 1)->where('active', 1)->orderByRaw('CAST(order_no AS UNSIGNED) ASC')->get();
        ...
        if (Cache::has('victorian_jewelry')) {
            ...
            $victorian_jewelry = Collection::where('parent', 4)->where('status', 1)->where('active', 1)->orderByRaw('CAST(order_no AS UNSIGNED) ASC')->get();
                    ...
                    if (!empty($collect)) {
                        foreach ($collect as $key => $col) {
                            ...
                            if ($coll) {
                                ...
                                $cols[$key]['order_no'] = $coll->order_no;
                            }
                        }
                        usort($cols, function ($a, $b) {
                            return intval($a['order_no']) <=> intval($b['order_no']);
                        });
                    }
                } else {
                    $cols = [];
    ...
    public function index()
    {
        ...
        // $banner = Banner::where('active', 1)->orderBy('order_no')->first();
        ...
        $collections_product = Collection::where('status', 1)
            ...
            ->with('productDetail')
            ->get();
        return view('frontend.index', compact('banner1', 'banner2', 'banner3', 'banner4', 'collections_product'));
    }
    ...
    public function registerPost(Request $request)
    {
        ...
        // Mail::to($request->email)->queue(new UserWelcomeMail($user));
        // return redirect()->back()->with('success', 'Thank you for connection with us!');
        return redirect()->route('my_account')->with('success', 'Thank you for connection with us!');
    }
            ...
            if ($request->has('id')) {
                ...
                if (!blank($products)) {
                    ...
                    foreach ($products as $product) {
                        ...
                        $html .= '<a href="' . route('product.details', $product->sku) . '" class=" text-center">';
    ...
    public function all_productDetails(Request $request)
    {
        try {
            ...
            $pro_collections = Collection::where('status', 1)->where('active', 1)->orderByRaw('CAST(order_no AS UNSIGNED) ASC')->get();
    ...
    function forgetPasswordPost(Request $request)
    {
        ...
        Mail::send("frontend.emails.forget-password", ['token' => $token, 'user' => $user], function ($message) use ($request) {
    ...
    //     try {
    //         // Send email to admin
    //         // Mail::to('vandan.rss@gmail.com')->send(new ContactInquiryMail($requestData, $attachment));
    //         // Send confirmation email to the customer
    //         Mail::to($request->email)->send(new ContactInquiryMail($requestData));
    //         Mail::to(env('ADMIN_MAIL_RECEIVE_ID'))->send(new ContactInquiryMail($requestData));
    ...
    //     return redirect()->back()->with('success', 'Your contact request has been submitted successfully!. We will contact you shortly');
    // }
    ...
    public function submitContactForm(Request $request)
    {
        ...
        // Send email to admin
        Mail::to(env('ADMIN_MAIL_RECEIVE_ID'))->send(new ContactFormSubmitted($data));

        // Short delay
        sleep(1); // wait 1 second

        // // Send confirmation email to user
        Mail::to($request->email)->send(new ContactFormSubmitted($data));
    ...
    public function submitCatalogRequest(Request $request)
    {
        ...
        try {
            // Send email to admin
            // Mail::to('vandan.rss@gmail.com')->send(new CatalogRequestMail($requestData, $attachment));

            // Send confirmation email to the customer
            Mail::to($request->email)->send(new CatalogRequestMail($requestData, $attachment));
            Mail::to(env('ADMIN_MAIL_RECEIVE_ID'))->send(new CatalogRequestMail($requestData, $attachment));
        } catch (\Exception $e) {
            // Log the error message
            \Log::error('Email sending failed: ' . $e->getMessage());
        }
    ...
    public function search(Request $request)
    {
        ...
        try {
            ...
            // Get matching category and collection IDs
            $matchedCategoryIds = Category::where('name', 'like', '%' . $sq . '%')->pluck('id')->toArray();
            $matchedCollectionIds = Collection::where('name', 'like', '%' . $sq . '%')->pluck('id')->toArray();

            $query = Product::where('status', 1)
                ->where(function ($q) use ($sq, $matchedCategoryIds, $matchedCollectionIds) {
                    $q->where('sku', 'like', '%' . $sq . '%')
                        ->orWhere('name', 'like', '%' . $sq . '%')
                        ->orWhereIn('category', $matchedCategoryIds)
                        ->orWhereIn('collection', $matchedCollectionIds);
                })
                ->orderBy($order_by, $order);
