...
@section('content')
    <section class="product_listing product_listing_outer_div">
        <!--<div class="container-fluid p-0 product_listing_innerbanner" style="background-image: url('{{ isset($category) ? (substr($category->image, 0, 7) == 'http://' || substr($category->image, 0, 7) == 'https://' ? $category->image : URL::asset('public/categories/' . $category->image)) : (substr($collection->image, 0, 7) == 'http://' || substr($collection->image, 0, 7) == 'https://' ? $collection->image : URL::asset('collection/' . $collection->image)) }}'); background-size: cover; background-position: center;">
         
        </div> -->
        <div class="container-fluid p-0 product_listing_innerbanner">
            <img src="{{ isset($category) ? (substr($category->image, 0, 7) == 'http://' || substr($category->image, 0, 7) == 'https://' ? $category->image : URL::asset('public/categories/' . $category->image)) : (substr($collection->image, 0, 7) == 'http://' || substr($collection->image, 0, 7) == 'https://' ? $collection->image : URL::asset('collection/' . $collection->image)) }}"
                class="img-fluid" alt="...">
        </div>
        <div class="container">
                ...
                <div class="product_listing_left">
                    <form id="filterForm">
                        <div class="">
                            ...
                            <div class="my-3">
                                <input type="radio" id="new" name="selection" value="new" class="selection"
                                    checked>
                            ...
                            @if (isset($category))
                                <h5 class=""> Sort By Collection</h5>
                            @else
                                <h5 class=""> Sort By Category</h5>
                            @endif
                            <div class="my-3">
                                {!! $cols !!}
                                        ...
                                        <div class="product_listing_inner">
                                            <a href="{{ route('product.details', $product->sku) }}" class=" text-center">
                                                <div class="product_listing_box">
                                                    ...
                                                    <p>{{ $product->name }}</p>
                                                    <h6>€ {{ $product->getPriceForCustomer() }}</h6>
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '.product_listing_pagination nav a', function(e) {
            e.preventDefault();
